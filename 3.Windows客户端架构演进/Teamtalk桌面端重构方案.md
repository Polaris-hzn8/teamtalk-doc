...
在什么情况下需要将功能代码单独拆分出动态库、单独编译？然后其他的程序\代码调用时，采用隐式、显式调用的方式来进行调用：

拆分原则：

1. 如果多个模块需要共享代码块：可以考虑做成dll动态库
2. 如果只有一个模块使用，可以直接编译进可执行程序、或动态库中
3. 如果大小很大，独立编译成dll可以减少编译的时间
4. 如果很小很简单，可以考虑直接包含源码更简单

| 问题                         | 答案 → 决策           |
| :--------------------------- | :-------------------- |
| 这个模块会被多个项目用到吗？ | 是 → DLL，否 → 源文件 |
| 这个模块经常改动吗？         | 是 → 源文件，否 → DLL |
| 这个模块体积大吗？           | 大 → DLL，小 → 源文件 |
| 有独立的维护团队吗？         | 是 → DLL，否 → 源文件 |

| 类型           | 生成文件    | 链接时机   | 运行时依赖     | 复杂度 |
| :------------- | :---------- | :--------- | :------------- | :----- |
| 动态库 DLL     | .dll + .lib | 运行时加载 | 需要 .dll 文件 | 高     |
| 静态库 .lib    | .lib        | 编译时合并 | 无             | 低     |
| 直接引入源文件 | 无          | 编译时合并 | 无             | 最低   |
